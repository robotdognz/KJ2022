...
namespace Together.Levels
{
    public class ShadowTrigger : MonoBehaviour
    {
        ...
        public float LowPriorityFraction = 2;
        ...
        private List<List<Vector2>> HitPositions
        {
            get
            {
                ...
                Actors.CameraController Camera = FindObjectOfType<Actors.CameraController>();

                foreach (GameObject G in GameObject.FindGameObjectsWithTag("ShadowCaster"))
                {
                    if (!Application.isPlaying || Vector2.Distance(Camera.PlayerCamera.position, G.transform.position) - ShadowDistance < Camera.PlayerCamera.GetComponent<Camera>().orthographicSize || 
                        Vector2.Distance(Camera.ShadowCamera.position, G.transform.position) - ShadowDistance < Camera.PlayerCamera.GetComponent<Camera>().orthographicSize)
                    { 
                        Transform Trans = G.transform;

                        int FinalCasterCount = CasterAmount;

                        if (G.name.Contains("LOWIMPORTANCE"))
                            FinalCasterCount = Mathf.RoundToInt(FinalCasterCount / LowPriorityFraction);

                        for (int I = 0; I < FinalCasterCount; I++)
                        {
                            float Radians = ((360f / FinalCasterCount) * I) / (180 / Mathf.PI);
